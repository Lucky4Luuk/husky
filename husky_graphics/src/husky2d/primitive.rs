...
pub fn add_methods<'lua, M: UserDataMethods<'lua, crate::Renderer>>(methods: &mut M) {
    ...
    methods.add_method("circle", |_, obj, (mode_raw, x,y, r): (String, f32,f32, f32)| {
        ...
        let color = *obj.active_color.lock().unwrap();
        obj.renderer2d.lock().unwrap().circle(color, mode, draw_x,draw_y, draw_w, draw_h);
        Ok(())
    });
    ...
    methods.add_method("tri", |_, obj, (mode_raw, x,y, w,h): (String, f32,f32, f32,f32)| {
        ...
        let draw_h = (h / win_size.1 as f32) * 2.0;
        let color = *obj.active_color.lock().unwrap();
        obj.renderer2d.lock().unwrap().tri(color, mode, draw_x,draw_y, draw_w,draw_h);
        Ok(())
    });
}
...
impl super::Renderer2D {
    ...
    pub fn circle(&mut self, color: (f32, f32, f32, f32), mode: Drawmode2D, x: f32, y: f32, w: f32, h: f32) {
        let scale = vec3(w,h,1f32);
        let translation = vec3(x,y,1f32);
        let model = Mat4::from_scale_rotation_translation(scale, Quat::IDENTITY, translation);
    ...
    pub fn tri(&mut self, color: (f32, f32, f32, f32), mode: Drawmode2D, x: f32, y: f32, w: f32, h: f32) {
        let scale = vec3(w,h,1f32);
        let translation = vec3(x,y,1f32);
        let model = Mat4::from_scale_rotation_translation(scale, Quat::IDENTITY, translation);
